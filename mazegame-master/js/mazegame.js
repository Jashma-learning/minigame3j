...
var createPlayerModel = function() {
    ...
    // Arms
    var armGeometry = new THREE.CylinderGeometry(0.05, 0.05, 0.4, 8);
    var armMaterial = new THREE.MeshPhongMaterial({ color: 0x3366cc });
...
var Game = function(args)
{
    ...
    this.player = {
        position: new THREE.Vector3( -1.5, 0.1, 1 )  // Restore original starting position
            .multiply( SCALE ),
        theta: Math.PI * 1.5,
        phi: 0,
        model: createPlayerModel()  // Add player model
    };
    
    // Improve camera settings without changing position
    camera.fov = 75;
    camera.near = 0.1;
    camera.far = 100;
    ...
    // Modify ambient light color and intensity for better atmosphere
    var light = new THREE.AmbientLight( 0xA0B0FF, 0.6 ); // Soft blue ambient light
    scene.add( light );

    // Make player light warmer and brighter
    this.player.light = new THREE.PointLight( 0xFFAA44, 0.7 * SCALE.average(), 2.5 * SCALE.average() );
    ...
    this.player.update = function()
    {
        ...
        this.model.rotation.y = this.theta;
        
        // Offset camera position to be behind player
        var cameraOffset = new THREE.Vector3(0, 0.7, 0.5).multiply(SCALE);
        this.dolly.position.add(cameraOffset);
    };
    ...
    var CubeMaterial = new THREE.MeshPhongMaterial( {
        color: 0x8899AA,  // Blue-gray walls
        bumpMap: CubeBumpMap,
        bumpScale: 0.55,
        shininess: 15,
    ...
    var CeilingMaterial = new THREE.MeshPhongMaterial( {
        color: 0x99AACC,  // Lighter blue-gray ceiling
    ...
    var FloorMaterial = new THREE.MeshPhongMaterial( {
        color: 0x778899,  // Darker blue-gray floor
    ...
    var OutsideFloor = new THREE.Mesh( 
        ...
        new THREE.MeshBasicMaterial( {
            ...
            color: 0x667788  // Dark blue-gray outside floor
        } )
    );
    ...
    this.player.model.scale.multiplyScalar(SCALE.average());
    scene.add(this.player.model);
};
...
Game.prototype.update = function( delta )
{
    ...
    if (InputManager.isKeyDown(87 /* w */) || 
        ...
        // Simple walking animation by rotating arms
        var time = performance.now() * 0.003;
        var armSwing = Math.sin(time) * 0.2;
        
        this.player.model.children[2].rotation.z = Math.PI / 6 + armSwing; // Left arm
        this.player.model.children[3].rotation.z = -Math.PI / 6 - armSwing; // Right arm
    }
};
